    ...
    return (
      <div className="w-full space-y-2">
        ...
        <input
          ...
          id={inputId}
          className={cn(
            "flex h-11 w-full rounded-lg border bg-background px-4 py-2 text-sm transition-all duration-300",
            "placeholder:text-muted-foreground",
            "focus:outline-none focus:ring-2 focus:ring-primary/30 focus:border-primary shadow-sm",
            "disabled:cursor-not-allowed disabled:opacity-50",
            error
              ? "border-destructive focus:ring-destructive/30 focus:border-destructive shadow-sm"
              : "border-input hover:border-muted-foreground/70 shadow-sm",
            className
          )}
          ref={ref}
          aria-invalid={!!error}
